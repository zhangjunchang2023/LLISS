...
class LLRGBD_real(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # load image
        img = Image.open(self.image_list[item])
        img = img.resize((self.crop_height , self.crop_width), Image.BILINEAR)
        img = np.array(img)
        ...
        img2 = Image.open(self.image_list2[item])
        img2 = img2.resize((self.crop_height , self.crop_width), Image.BILINEAR)
        img2 = np.array(img2)
        ...
        label = Image.open(self.label_list[item])
        label = label.resize((self.crop_height , self.crop_width), Image.NEAREST)
        label = np.array(label)
        label = torch.from_numpy(label).long()
