...
def tensor_to_image(tensor):
    """
    将 shape 为 [C, H, W] 的 tensor 转为 uint8 RGB 图像。
    """
    if isinstance(tensor, torch.Tensor):
        tensor = tensor.detach().cpu().numpy()

    # 如果是灰度图，只取第一个通道重复3次变成RGB
    if tensor.shape[0] == 1:
...
def save_checkpoint(params, is_best=False, directory='weights/', filename='checkpoint.pth.tar'):
...
def colorize(x):
    """
    将单通道语义分割 mask 映射为 RGB 颜色图
    输入 x: torch.Tensor, shape: (H, W)
    输出: numpy.ndarray, shape: (H, W, 3), dtype: uint8
    """
    ...
    x = x.detach().cpu().numpy().astype(int)   # 转成 NumPy int 类型
    x_color = colour_code[x]                   # 根据标签值映射颜色
    return x_color                             # 返回的是 (H, W, 3)


# def colorize(x):
#     colour_code = np.array([[0, 0, 1],
#                        [0, 1, 0],
#                        [1,0,0] ])
#
...
def rgb_to_label(label_rgb):
    """
    将 RGB 标签图转换为单通道整数类别标签。
    label_rgb: Tensor, shape [B, H, W, 3] or [H, W, 3]
    返回: Tensor, shape [B, H, W] or [H, W]
    """
    color_map = {
        (0, 0, 1): 0,   # 蓝色 → 类别0
        (0, 1, 0): 1,   # 绿色 → 类别1
    ...
    label_rgb = label_rgb.detach().cpu().numpy()
    label = np.zeros(label_rgb.shape[:3], dtype=np.uint8)

    for rgb, class_id in color_map.items():
        mask = np.all(label_rgb == np.array(rgb), axis=-1)
        label[mask] = class_id
